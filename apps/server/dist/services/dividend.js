...
function getCurrentCycleDueDate(now) {
...
export async function invalidateEstimatedDividendPerShare(year) {
    const redis = getRedis();
    const y = year ?? (await getCurrentYear());
    await redis.del(REDIS_KEYS.estimatedDividendPerShare(y));
}
export async function getEstimatedDividendPerShare() {
    const redis = getRedis();
    ...
    const eligibleUserIds = cycleEligibilities.map((e) => e.userId);
    ...
    const totalEligibleShares = users.reduce((acc, u) => {
        const hasValidPayment = latestByUser.has(u.id);
        if (hasValidPayment)
            return acc + (u.share_count || 0);
        return acc;
    }, 0);
