...
router.get('/users', async (req, res) => {
    ...
    activeLoansByUser.forEach((r) => {
        if (r.userId != null)
            loanMap.set(r.userId, r._count._all);
    });
    const coMakerMap = new Map();
    coMakerByUser.forEach((r) => {
        coMakerMap.set(r.userId, r._count._all);
    });
    const usersWithStatus = users.map((u) => {
        const expectedAmount = u.share_count * (config?.share_value || 0);
        const totalPaidCurrentDue = (u.contributions || []).reduce((sum, c) => sum + (c.amount || 0), 0);
        let contribution_status = 'NO_PAYMENT';
        ...
        let loan_payment_status = 'NO_LOAN';
        if (activeLoans.length > 0) {
            ...
            if (hasLate)
                loan_payment_status = 'LATE';
            else if (hasPartial)
                loan_payment_status = 'PARTIAL';
            else
                loan_payment_status = 'ON_TIME';
        }
        return {
            id: u.id,
...
router.get('/dashboard', async (_req, res) => {
    ...
    users.forEach((u) => {
        ...
        const totalPaid = u.contributions.reduce((sum, c) => sum + c.amount, 0);
...
router.post('/loans', async (req, res) => {
    ...
    const { borrowerType, userId, borrowerName, borrowerEmail, borrowerPhone, principal, termMonths, coMakers = [], } = parsed.data;
...
router.post('/users/bulk-passwords', async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ error: parsed.error.flatten() });
    const whereClause = { id: { in: parsed.data.userIds } };
...
router.post('/archive-run', async (req, res) => {
    ...
    const performedByUserId = req.adminUserId ?? null;
...
router.get('/dividends/payouts', async (req, res) => {
    ...
    const where = {};
    if (q.data.year)
        where.year = Number(q.data.year);
    if (q.data.userId)
        where.userId = Number(q.data.userId);
...
router.post('/dividends/payouts', async (req, res) => {
    ...
    const createdByUserId = req.adminUserId ?? null;
    try {
